...
export default function StudentDetailPage() {
  ...
  const durationData = useMemo(() => {
    ...
    return Array.from({ length: 7 }, (_, i) => {
      ...
      const mins = Math.round(
        sessions.filter(s => s.date === ds && s.type === 'work')
  ...
  const handleRestorePlan = (plan: PendingPlan) => {
    ...
    toast.success('Plan taslağa yüklendi, düzenleyebilirsiniz');
  };
  ...
  // Build day list for date selector
  const buildDayList = () => {
    ...
    <div className="px-4 pt-6 pb-24">
      ...
      <Tabs defaultValue="plan">
        ...
        <TabsContent value="plan" className="space-y-4">
          ...
          {/* Student's existing tasks for this date */}
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {studentTasksForDate.map(task => (
                  <div key={task.id} className={`px-3 py-2.5 rounded-xl border text-sm ${
                    task.source === 'teacher' ? 'bg-primary/5 border-primary/20' : 'bg-card border-border'
                  }`}>
                    <div className="flex items-center gap-2">
                      <span className="text-card-foreground">{task.name}</span>
                      {task.source === 'teacher' && (
                        <Badge variant="outline" className="text-[9px] px-1.5 py-0 border-primary/30 text-primary">Koç</Badge>
                      )}
                    </div>
        ...
        <TabsContent value="archive" className="space-y-3">
          {plans.length === 0 ? (
            ...
            plans.map(plan => {
              ...
              const statusLabel = plan.status === 'pending' ? 'Bekliyor' : plan.status === 'accepted' ? 'Onaylandı' : 'Reddedildi';
              const statusColor = plan.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : plan.status === 'accepted' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
              return (
                <div key={plan.id} className="bg-card rounded-2xl p-4 border border-border shadow-sm space-y-2">
                  ...
                  {planDates.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {planDates.map(d => (
                        <span key={d} className="text-[10px] px-2 py-0.5 bg-accent rounded-lg">{d}</span>
                      ))}
                    </div>
                  )}
