...
import { Download, Trash2, Sun, Moon, LogOut, Save, User, Copy, Check, UserPlus, GraduationCap, ArrowLeftRight, Lock, Eye, EyeOff, ChevronRight, MessageSquare, Send, Search, Palette, Bell, Clock, Target, Database } from 'lucide-react';
import {
...
type SettingsSection = 'main' | 'profile' | 'theme' | 'planning' | 'coach' | 'data';
...
export default function SettingsPage() {
  ...
  const handleExport = () => {
    ...
    a.href = url; a.download = `checktime-export-${new Date().toISOString().split('T')[0]}.json`;
  ...
  const handleSendRequest = async () => {
    ...
    setSending(false);
    if (error) { toast.error('İstek gönderilemedi: ' + error); }
    else { toast.success('İstek gönderildi!'); setCoachDialogOpen(false); setRefCode(''); setLookupResult(null); refetchCoach(); }
  };
  ...
  const openTeacherChat = async (rel: { id: string; teacher_id: string; teacher_name?: string }) => {
    ...
    const { data } = await supabase.from('pending_plans').select('*').eq('teacher_id', rel.teacher_id).eq('student_id', user?.id || '').order('created_at', { ascending: false }).limit(1);
    if (data && data.length > 0) { setChatPlanId(data[0].id); setChatMessages(Array.isArray((data[0] as any).messages) ? (data[0] as any).messages : []); }
    else {
      const { data: newPlan } = await supabase.from('pending_plans').insert({ teacher_id: rel.teacher_id, student_id: user?.id || '', plan_data: [], status: 'chat', messages: [] }).select().single();
  ...
  // ─── MAIN MENU ───
  if (section === 'main') {
    const menuItems: { id: SettingsSection; icon: any; label: string; desc: string; show: boolean }[] = [
      { id: 'profile', icon: User, label: 'Kullanıcı İşlemleri', desc: 'Profil, rol, şifre, çıkış', show: true },
      { id: 'theme', icon: Palette, label: 'Tema & Görünüm', desc: 'Renk paleti, mod', show: true },
      { id: 'planning', icon: Clock, label: 'Planlama Ayarları', desc: 'Saat, mod, alan seçimi', show: isStudentMode },
      { id: 'coach', icon: GraduationCap, label: isTeacherMode ? 'Öğrencilerim' : 'Koç İşlemleri', desc: isTeacherMode ? 'Öğrenci listesi, istekler' : 'Koç bağlantısı, referans', show: isExamOrUni || isTeacherMode },
      { id: 'data', icon: Database, label: 'Veri Yönetimi', desc: 'Export, sıfırlama', show: true },
    ];

    return (
      <div className="px-4 pt-6 pb-24">
        <h1 className="text-lg font-bold mb-4 text-foreground">Ayarlar</h1>
        ...
        <div className="bg-card rounded-2xl p-4 border border-border shadow-sm mb-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-foreground truncate">{profile?.display_name || 'İsim belirle'}</p>
            ...
            {hasMultipleRoles && (
              <Select value={activeRole} onValueChange={setActiveRole}>
                ...
                <SelectContent>
                  {getRoleOptions().map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}
                </SelectContent>
              </Select>
            )}
          </div>
        ...
        <div className="space-y-2">
          {menuItems.filter(m => m.show).map(item => (
            <button key={item.id} onClick={() => setSection(item.id)}
              className="flex items-center gap-3 w-full px-4 py-3.5 bg-card rounded-xl border border-border hover:bg-accent transition-colors text-left">
  ...
  const BackButton = () => (
    <button onClick={() => setSection('main')} className="text-sm text-primary font-medium mb-4 flex items-center gap-1 hover:opacity-80">
      ← Geri
    </button>
  );
  ...
  if (section === 'profile') {
    return (
      <div className="px-4 pt-6 pb-24">
        <BackButton />
        <h2 className="text-lg font-bold mb-4 text-foreground">Kullanıcı İşlemleri</h2>
        <div className="space-y-4">
          <div className="bg-card rounded-2xl p-4 border border-border shadow-sm space-y-3">
            <h3 className="text-sm font-semibold text-card-foreground">Profil</h3>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-sm">
                {(profile?.display_name || user?.email || '?')[0].toUpperCase()}
              </div>
            ...
            {hasMultipleRoles && (
              <div className="flex items-center justify-between pt-2 border-t border-border">
                ...
                <Select value={activeRole} onValueChange={setActiveRole}>
                  ...
                  <SelectContent>{getRoleOptions().map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" size="sm" onClick={async () => { await signOut(); }} className="flex-1 rounded-xl text-destructive border-destructive/30 hover:bg-destructive/5">
  ...
  if (section === 'theme') {
    return (
      <div className="px-4 pt-6 pb-24">
        <BackButton />
        <h2 className="text-lg font-bold mb-4 text-foreground">Tema & Görünüm</h2>
        <div className="space-y-4">
          <div className="bg-card rounded-2xl p-4 border border-border shadow-sm space-y-3">
  ...
  if (section === 'planning') {
    return (
      <div className="px-4 pt-6 pb-24">
        <BackButton />
        <h2 className="text-lg font-bold mb-4 text-foreground">Planlama Ayarları</h2>
        <div className="space-y-4">
          <div className="bg-card rounded-2xl p-4 border border-border shadow-sm space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Select value={String(settings.planningHourStart ?? 8)} onValueChange={v => updateSettings({ planningHourStart: parseInt(v) })}>
                  ...
                  <SelectContent className="max-h-48">{Array.from({ length: 25 }, (_, i) => <SelectItem key={i} value={String(i)}>{String(i).padStart(2, '0')}:00</SelectItem>)}</SelectContent>
                </Select>
          ...
          {isExamOrUni && (
            <div className="bg-card rounded-2xl p-4 border border-border shadow-sm space-y-3">
              <h3 className="text-sm font-semibold text-card-foreground">Alan Seçimi</h3>
  ...
  if (section === 'coach') {
    return (
      <div className="px-4 pt-6 pb-24">
        <BackButton />
        <h2 className="text-lg font-bold mb-4 text-foreground">{isTeacherMode ? 'Öğrencilerim' : 'Koç İşlemleri'}</h2>
        <div className="space-y-4">
          {/* Referral code */}
          <div className="bg-card rounded-2xl p-4 border border-border shadow-sm space-y-3">
          ...
          {/* Connected teachers (student) */}
          {isStudentMode && acceptedTeachers.length > 0 && (
            <div className="bg-card rounded-2xl p-4 border border-border shadow-sm space-y-2">
              <h3 className="text-sm font-semibold text-card-foreground">Koçum</h3>
              {acceptedTeachers.map(rel => (
                <button key={rel.id} onClick={() => openTeacherChat(rel)}
          ...
          {isTeacherMode && (
            <div className="bg-card rounded-2xl p-4 border border-border shadow-sm space-y-3">
              ...
              {filteredStudents.map(rel => (
                <button key={rel.id} onClick={() => navigate(`/student/${rel.student_id}`)}
              ...
              {filteredStudents.length === 0 && <p className="text-xs text-muted-foreground text-center py-2">Henüz öğrenci yok</p>}
            </div>
          )}
          ...
          {pendingRequests.length > 0 && (
            <div className="bg-card rounded-2xl p-4 border border-border shadow-sm space-y-2">
              <h3 className="text-sm font-semibold text-card-foreground">Bekleyen İstekler</h3>
              {pendingRequests.map(req => (
                <div key={req.id} className="bg-accent/30 rounded-xl p-3 space-y-2">
                  <p className="text-xs text-foreground">
                    <strong>{isTeacherMode ? req.student_name : req.teacher_name || 'İsimsiz'}</strong> {isTeacherMode ? 'sizinle çalışmak istiyor.' : 'adlı öğretmen sizinle çalışmak istiyor.'}
                  </p>
                  <div className="flex gap-2">
  ...
  if (section === 'data') {
    return (
      <div className="px-4 pt-6 pb-24">
        <BackButton />
        <h2 className="text-lg font-bold mb-4 text-foreground">Veri Yönetimi</h2>
